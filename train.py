"""Compare a majority-class baseline with a Random Forest on Titanic records."""
...
def load_frame(path: Path) -> pd.DataFrame:
    if not path.is_file():
        raise FileNotFoundError(
            f"Real Titanic data not found at {path}. Download the required train.csv first."
        )
...
def verify_real_data(path: Path) -> dict[str, object]:
    """Check that the downloaded classroom data has the expected structure."""
...
def preprocessor() -> ColumnTransformer:
    ...
    return ColumnTransformer(
        [("category", categorical, CATEGORICAL), ("number", numeric, NUMERIC)]
    )


def build_baseline() -> Pipeline:
    return Pipeline(
        [("prepare", preprocessor()), ("model", DummyClassifier(strategy="most_frequent"))]
    )


def build_candidate() -> Pipeline:
    """Build the reproducible candidate model required by the assignment."""
    return Pipeline(
        [
...
def evaluate(y_true: pd.Series, y_pred: object) -> dict[str, object]:
    ...
    return {
        ...
        "precision_survived": float(precision_score(y_true, y_pred, pos_label=1, zero_division=0)),
        "recall_survived": float(recall_score(y_true, y_pred, pos_label=1, zero_division=0)),
...
def main() -> int:
    ...
    errors = frame.loc[x_test.index, ["PassengerId", "Name", *FEATURES]].copy()
